    ...
    <div className="h-full flex flex-col bg-brand-espresso text-white w-64 border-r border-white/[0.06]">
      ...
      <nav className="flex-1 p-3 space-y-0.5 overflow-y-auto">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={`flex-shrink-0 transition-transform duration-150 group-hover:scale-110 ${isActive ? "text-white" : "text-white/50 group-hover:text-white"}`}>
                {item.icon}
              </span>
              {item.label}
            </Link>
      ...
      <div className="p-3 border-t border-white/10 space-y-0.5">
        <Link
          ...
          onClick={onClose}
          target="_blank"
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-white/50 hover:text-white hover:bg-white/8 transition-all duration-150"
        >
          <span className="flex-shrink-0"><EyeIcon /></span>
          View Menu
        </Link>
        <button
